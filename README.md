...
**The synthetic env fails its own audit — disclosed, not hidden.** Running `find_deterministic_edges` on the synthetic metadata flags `vpc_cidr` and `iam_role` at **1.000 determinism** of team — the same pattern we called a leak on Azure. This is by construction: the env deliberately models real org structure (each team owns a VPC; role names follow team conventions), and `build_graph` uses those as edges. The honest distinction the audit alone can't make: on Azure the determinism was a *benchmark artifact* to exclude; here it's *modeled world-structure* — but by our own numeric standard, both trigger. Two mitigations keep the synthetic numbers meaningful: (1) the [edge-dropout sweep](docs/robustness.md) shows **89.4% with the graph entirely removed**, so the headline doesn't depend on the deterministic edges; (2) this is exactly why synthetic results are labeled *ablation, not headline*. An earlier version of this README claimed these checks "don't trigger" — that was wrong, and this correction is itself the audit working. (Pinned by a regression test so the claim can't silently drift again.)
